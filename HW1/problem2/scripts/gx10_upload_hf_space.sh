...
if [[ $# -lt 1 ]]; then
  ...
  echo "Example: $0 jimmy60504/QML-Classifier-Explorer main" >&2
  exit 1
fi
...
SPACE_OUTPUT_DIR="${HF_SPACE_OUTPUT_DIR:-${REPO_ROOT}/.out/hf_space_hw1_problem2_publish}"
...
"${REPO_ROOT}/HW1/problem2/scripts/gx10_prepare_hf_space.sh"
...
"${HF_BIN}" upload \
  ...
  --revision "${REVISION}" \
  --exclude ".cache/**" \
  --commit-message "${HF_SPACE_COMMIT_MESSAGE:-Update QML Classifier Explorer}"
