...
**瓶頸分析：** SA 的主要困難不是退火排程本身，而是 **8 個 slack bit 把搜尋空間從 $2^{10}$ 膨脹到 $2^{18}$**。大多數 reads 會花在跟物品選擇無關的 slack 子空間裡，因此即使最佳解偶爾會出現，命中率仍然很低。
...
受 Branch-and-Bound 剪枝啟發，我們實驗了一種採樣式的「排除剪枝」：每輪用少量 shots 跑 SA，找出最可能為 0（不選）的 bit 固定掉，逐步縮小問題規模，最後對剩餘小問題做古典暴力解。
...
IEQ 面臨一個根本矛盾：需要可靠量測才能安全剪枝，但量測可靠通常也代表 SA 已經足夠好，直接採樣反而更有效率。比較可能有利的區間，是 SA 命中率低但不是 0 的中等規模問題（例如命中率約 1–5%）：過濾後的訊號仍可信，每輪剪枝又能提升下一輪命中率，才可能形成正循環。N=10 的背包問題太小，落不在這個區間。
