...
**終止即最優：** Phase 3 對所有連通子圖的生長路徑都無法找到改善，等同於對所有「晶體旋轉」操作都做了隱式排查。不需要外部上界（如 SDP）——連動的晶體找不到了，就是停止條件。
...
- 梯度下降在 p=1 的粗糙景觀中容易陷入局部最優，但因為景觀有規律性，多次嘗試可以找到最優方向
...
Max-Cut 是一個二次（2-local）問題，QAOA 在 $p=2$ 時就能穩定地找到 8 節點圖的最優解，這與問題的局部結構相對規整有關。相比之下，SA 因為搜尋空間只有 $2^8=256$，幾乎任何採樣次數都可以命中最優。
...
傳統 k-opt 需要外部上界（SDP、$|E|$）來確認「已接近最優」。Chain-Crystal 的終止條件本身就是最優性的隱式保證：

> **若所有種子的 chain 生長路徑都找不到 ΔC > 0，代表圖中不存在任何連通子圖的旋轉可以改善切割——域壁已在最優位置。**

這對應 QAOA 的量子意義：若增加電路深度 p 後能量不再下降，代表更大範圍的量子關聯已無法進一步最佳化當前 Hamiltonian。
...
| Chain 找不到改善 → 終止 | 增加 $p$ 能量不再下降 |
| 晶體尺寸上限 max_chain | 電路深度上限 |

QAOA 在 $p=2$ 找到最優的原因：這張圖最難的局部最優只需「翻轉 2 個連通節點」就能逃脫，Chain-Crystal 實驗中 N=8 最大晶體尺寸也以 2 為主，兩者完全對應。
